...
def postulacion(request):
    ...
    return render(request, 'voting/postulacion.html')


def postulacion_exitosa(request):
    """Página de éxito tras enviar la postulación"""
    return render(request, 'voting/postulacion_exitosa.html')


@require_http_methods(["POST"])
def enviar_postulacion(request):
    """
    Recibe el formulario de postulación, envía correo con adjuntos a los
    destinatarios y elimina los archivos temporales del servidor.
    """
    ...
    nombres_apellidos = request.POST.get('nombres_apellidos', '').strip()

    if not nombres_apellidos:
        messages.error(request, 'El campo "Nombres y Apellidos" es obligatorio.')
        return redirect('voting:postulacion')
    ...
    doc_fields = [
        ...
        ('doc_patrocinantes', 'Formulario de Patrocinantes'),
        ('doc_form_cabildo',  'Formulario Inscripción de Candidaturas Cabildo'),
    ]

    # Validar que todos los documentos estén presentes
    for field_name, label in doc_fields:
        if field_name not in request.FILES or not request.FILES[field_name]:
            messages.error(request, f'El documento "{label}" es obligatorio.')
            return redirect('voting:postulacion')

    # Guardar archivos en archivos temporales para adjuntarlos y luego borrarlos
    temp_files = []
    doc_names = []
    ...
    try:
        for field_name, label in doc_fields:
            ...
            tmp.flush()
            tmp.close()
            temp_files.append((tmp.name, uploaded.name, uploaded.content_type or 'application/octet-stream'))
            doc_names.append(uploaded.name)

        fecha_envio = datetime.now().strftime('%d/%m/%Y %H:%M:%S')

        # Renderizar HTML del correo
        html_body = render_to_string('voting/emails/postulacion_email.html', {
        ...
        destinatarios = [
            'tribunalsupremo@partidorepublicanodechile.cl',
            'matias.barrios.wow@gmail.com',
        ]

        email_msg = EmailMessage(
            subject=f'Nueva Postulación de {nombres_apellidos}',
            body=plain_body,
            from_email=settings.DEFAULT_FROM_EMAIL,
            to=destinatarios,
        )
        email_msg.content_subtype = 'html'
        email_msg.body = html_body

        # Adjuntar archivos temporales
        for tmp_path, original_name, content_type in temp_files:
            with open(tmp_path, 'rb') as f:
                email_msg.attach(original_name, f.read(), content_type)

        email_msg.send(fail_silently=False)

    except Exception as e:
        messages.error(request, f'Ocurrió un error al enviar la postulación: {str(e)}')
        return redirect('voting:postulacion')

    finally:
        # Eliminar archivos temporales del servidor siempre
    ...
    return redirect('voting:postulacion_exitosa')
